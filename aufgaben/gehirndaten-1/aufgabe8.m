...
% Eingrenzung auf die interessanten Frequenzen (0.3-32Hz)

f_low = 0.3;
f_high = 32;

% Butterworth-Filter (4. Ordnung)
[b_high, a_high] = butter(order, f_low/fn, 'high');   % Hochpass
[b_low, a_low] = butter(order, f_high/fn, 'low');     % Tiefpass

% Anwendung auf detrendetes Signal (z. B. y_detrend1 von oben)
y_band = filtfilt(b_low, a_low, filtfilt(b_high, a_high, y_detrend1));

% Gesamtsignal im Zeitbereich plotten
figure(9);
plot(x, y_band);
title('Bandpass-gefiltertes Signal (0.3–32 Hz)');
xlabel('Zeit (s)');
ylabel('Messwerte');
...
samples_zoom = fsample * duration_zoom;
x_zoom = x(1:samples_zoom);
y_zoom = y_band(1:samples_zoom);
figure(10);
plot(x_zoom, y_zoom);
title('Bandpass-Signal (0.3–32 Hz) – Erste 3 Sekunden');
xlabel('Zeit (s)');
ylabel('Amplitude');

% FFT des bandpassgefilterten Signals
Y_band = 1/rows * abs(fft(y_band));
Y_band = Y_band(1:rows2); Y_band(2:end-1) = 2 * Y_band(2:end-1);

figure(11);
stem(f(2:rows2), Y_band(2:end), 'LineWidth', 2);
xlabel('Frequenz (Hz)');
ylabel('Amplitude');
title('FFT Bandpass-gefiltertes Signal (0.3–32 Hz)');
grid on;
xlim([0.1 100]);
